...
type ItemEscala = EscalaCompleta["itens"][number];
...
const GABARITO_COLABORACAO: GabaritoColaboracao = {
  ...
  secoes: [
    ...
    },
    {
      ...
      isSecaoAdmin: true,
      isSecaoPalestrante: false,
      itemFuncoes: ["COORDENADOR", "AUXILIAR ADM.", "AUXILIAR ADM."],
    },
    {
      nome: "COMANDOS",
      total: 4,
      categoriaEsperada: "COMANDO",
      permiteMultiplosItens: true,
      isSecaoAdmin: false,
      isSecaoPalestrante: false,
      itemFuncoes: [
    ...
    },
    {
      nome: "CHEFE DE TURMA",
      total: 3,
      categoriaEsperada: "CHEFE_TURMA",
      permiteMultiplosItens: true,
      isSecaoAdmin: false,
      isSecaoPalestrante: false,
      itemFuncoes: [
        "CHEFE DE TURMA – 1ª CIA",
        "CHEFE DE TURMA – 2ª CIA",
        "CHEFE DE TURMA – 3ª CIA",
      ],
    },
    {
      nome: "GUARDA BANDEIRA",
      total: 3,
      categoriaEsperada: "GUARDA_BANDEIRA",
      permiteMultiplosItens: true,
      isSecaoAdmin: false,
      isSecaoPalestrante: false,
      itemFuncoes: ["GUARDA BANDEIRA", "GUARDA BANDEIRA", "GUARDA BANDEIRA"],
    },
    {
      nome: "ESCALA EXTRA",
      total: 5,
      categoriaEsperada: "ESCALA_EXTRA",
      permiteMultiplosItens: true,
      isSecaoAdmin: false,
      isSecaoPalestrante: false,
      itemFuncoes: [
...
private formatarNomeAluno(item: ItemEscala): string {
  const aluno = item.aluno;
  if (!aluno) return 'N/A';

  // O TypeScript agora sabe exatamente o que existe dentro de aluno
  // baseado no include que você fez no Prisma (perfilAluno -> cargo)
  const cargoAbrev = aluno.perfilAluno?.cargo?.abreviacao || "";
  const nomeGuerra = aluno.nomeDeGuerra || aluno.nome;

  // Monta a string: "AL GM NOME"
  return `${cargoAbrev} GM ${nomeGuerra}`.trim().toUpperCase();
}
  ...
  private async loadFonts(): Promise<void> {
    try {
      ...
      console.warn(
        "Erro ao carregar fontes personalizadas, usando fontes padrão:",
        error,
      );
  ...
  addHeader(): this {
    ...
    const diaFormatado = format(dataCorrigida, "dd/MM/yyyy (EEEE)", {
      locale: ptBR,
    }).toUpperCase();
    ...
    const texto =
      tituloPersonalizado && tituloPersonalizado.trim() !== ""
  ...
  private addScaleContent(): void {
    const secoes = this.escala.itens.reduce(
      (acc, item) => {
    ...
    const orderedSecoes = Object.keys(secoes).sort((a, b) => {
      const orderA = GABARITO_COLABORACAO.secoes.findIndex(
        (g) => g.nome.toUpperCase() === a,
      );
      const orderB = GABARITO_COLABORACAO.secoes.findIndex(
        (g) => g.nome.toUpperCase() === b,
      );
    ...
    for (const nomeSecao of orderedSecoes) {
      ...
      if (nomeSecao === "PALESTRANTE") {
        ...
        body = itensSecao.map((item) => [
          ...
          {
            content: this.formatarNomeAluno(item),
            styles: { halign: "center" },
          },
        ]);
      ...
      autoTable(this.doc as jsPDFWithAutoTable, {
        ...
        columnStyles: columnStyles as unknown as {
          [key: string]: Partial<TableStyles>;
        },
          ...
          if (data.section === "body") {
            ...
            if (isPalestrante && data.column.index === 2) {
              ...
              if (fullText.startsWith(prefix.toUpperCase())) {
                ...
                this.doc.setFont("Arial", "bold");
                this.doc.text(prefixToShow, textX, textY, {
                  baseline: "middle",
                });
                ...
                this.doc.setFont("Arial", "normal");
                this.doc.text(studentName, textX + prefixWidth, textY, {
                  baseline: "middle",
                });
              }
            }
      ...
      this.currentY =
        (this.doc as jsPDFWithAutoTable).lastAutoTable?.finalY ??
        this.currentY + 2;
    }

    const addSingleColumnSection = (
      title: string,
    ...
    const dataPublicacao =
      this.escala.publishedAt || this.escala.createdAt || new Date();
    const dataFormatada = new Date(dataPublicacao).toLocaleDateString("pt-BR", {
      day: "2-digit",
      month: "2-digit",
      year: "2-digit",
    });
    const adminResponsavel = this.escala.criadoPor as Usuario & {
      funcaoAdmin: FuncaoAdmin | null;
    };

    const nomeCompletoAdmin = adminResponsavel?.nome || "NOME DO ADMINISTRADOR";
    const nomeDeGuerraAdmin =
      adminResponsavel?.nomeDeGuerra || "NOME DE GUERRA";
    const funcaoAdminCompleta =
      adminResponsavel?.funcaoAdmin?.nome || "AUX ADM - ESCALANTE";
    ...
    autoTable(this.doc as jsPDFWithAutoTable, {
      ...
      columnStyles: {
        0: {
          cellWidth: (this.pageWidth - this.margin * 2) / 3,
          halign: "center",
        },
        1: {
          cellWidth: (this.pageWidth - this.margin * 2) / 3,
    ...
    this.doc.text(
      funcaoAdminCompleta.toUpperCase(),
      this.pageWidth / 2,
      this.currentY + 6,
      { align: "center" },
    );
