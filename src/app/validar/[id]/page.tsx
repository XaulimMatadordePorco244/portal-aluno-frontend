      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-6 border border-gray-200">
        ...
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-gray-900">
            <FormattedName fullName={aluno.nome} warName={aluno.perfilAluno?.nomeDeGuerra} />
          </h2>
          <p className="text-md text-gray-600">{aluno.perfilAluno?.cargo?.nome ?? 'Não informado'}</p>

          <p><strong>Cargo:</strong> </p>
          <p className="text-sm text-gray-500 mt-1">Nº: {aluno.perfilAluno?.numero || 'N/A'}</p>
        </div>

        <div className="mt-6">
          <p className="text-center text-sm text-gray-500 mb-2">Status da Matrícula</p>
          <Badge
            className={`w-full text-center flex items-center justify-center text-lg py-2 ${isAtivo ? 'bg-green-100 text-green-800 border-green-300' : 'bg-red-100 text-red-800 border-red-300'
              }`}
            variant="outline"
          >
            {aluno.status}
