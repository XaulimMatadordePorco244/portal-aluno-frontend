...
export function MapaFrequenciaClient({ alunos, frequencias, datas, tipo }: Props) {
  // Otimistic UI state seria ideal, mas vamos fazer simples com refresh do server action
  
  const getStatus = (alunoId: string, dataIso: string) => {
    // Atenção: dataIso vem como string "YYYY-MM-DD"
    // O banco retorna DateTime. Precisamos comparar só a data.
  ...
  const handleClick = async (alunoId: string, dataIso: string) => {
    ...
    // Feedback visual imediato pode ser complexo sem state local completo, 
    // mas o Server Action revalidatePath vai atualizar a tela.
    try {
