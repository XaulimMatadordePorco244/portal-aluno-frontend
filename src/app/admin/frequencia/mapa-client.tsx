...
export function MapaFrequenciaClient({ alunos, frequencias, datas, tipo }: Props) {
  const getStatus = (alunoId: string, dataIso: string): StatusFrequencia | null => {
    const reg = frequencias.find(
      f => f.alunoId === alunoId && f.data.toISOString().startsWith(dataIso)
    )
          ...
          <tr>
            ...
            {datas.map(d => {
              ...
              return (
                <th
                  key={d}
                  className="p-1 border-b border-r text-center min-w-[50px] bg-muted/95 backdrop-blur align-middle"
                >
                  <Tooltip>
