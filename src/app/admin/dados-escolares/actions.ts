...
export async function getTodosAlunos(
  ordenacao: "nome" | "guerra" | "antiguidade" = "nome",
) {
  let orderByClause:
    | Prisma.UsuarioOrderByWithRelationInput
    | Prisma.UsuarioOrderByWithRelationInput[] = { nome: "asc" };
  ...
  return await prisma.usuario.findMany({
    where: {
      ...
      perfilAluno: {
        is: {
          status: StatusPerfilAluno.ATIVO,
        },
      },
    },
    include: {
      perfilAluno: {
        include: { escola: true, cargo: true },
      },
    },
...
export async function salvarDadosEscolaresEmLote(
  atualizacoes: AtualizacaoEscolarData[],
  anoAtual: number,
) {
  try {
