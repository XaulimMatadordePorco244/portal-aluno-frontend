...
function initializeState(
  ...
  return Array.from(secoesMap.entries()).map(([nomeSecao, itensDaSecao]) => {
    const isSecaoPalestrante = itensDaSecao.some(
      (i) =>
        i.cargo === "PALESTRA" || i.observacao?.startsWith(AUXILIAR_PREFIX),
    );

    const isSecaoAdmin =
      itensDaSecao.length > 0 &&
      itensDaSecao.every((item) => admins.some((a) => a.id === item.alunoId));
    ...
    const itensState: ItemState[] = itensDaSecao.map((item) => {
      ...
      if (isSecaoPalestrante) {
        ...
        if (item.observacao && item.observacao.startsWith(AUXILIAR_PREFIX)) {
          ...
          const auxUser = alunos.find(
            (a) => a.nomeDeGuerra === auxNome || a.nome === auxNome,
          );
          auxiliarUserId = auxUser?.id || "";
        }
      } else {
        if (!funcaoId) {
          const funcaoMatch = funcoes.find(
            (f) => f.nome.toUpperCase() === item.cargo.toUpperCase(),
          );
          if (funcaoMatch) {
...
export function EditEscalaForm({
  ...
  const [fardamento, setFardamento] = useState(
    escalaInicial.fardamento || GABARITO_COLABORACAO.fardamento,
  );
  const [observacoes, setObservacoes] = useState(
    escalaInicial.observacoes || GABARITO_COLABORACAO.observacoes,
  );

  const funcaoOutroId = useMemo(
    () => funcoesProp.find((f) => f.nome.toUpperCase() === "OUTRO")?.id || "",
    [funcoesProp],
  );

  const [secoes, setSecoes] = useState<SecaoState[]>(() =>
    initializeState(escalaInicial, alunos, admins, funcoesProp, funcaoOutroId),
  );
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast.success("Sucesso!", {
        description:
          "As alterações foram salvas. O PDF foi invalidado e a escala retornou para Rascunho.",
      });
      setIsEditing(false);
      router.refresh();
    } catch (error) {
  ...
  const handlePreVisualizarPdf = async () => {
    ...
    try {
      const response = await fetch(
        `/api/escalas/${escalaInicial.id}/preview-pdf`,
        { method: "GET" },
      );
  ...
  const handlePublish = async () => {
    ...
    try {
      const response = await fetch(
        `/api/escalas/${escalaInicial.id}/publicar`,
        {
          method: "POST",
        },
      );
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {format(new Date(dataEscala), "dd 'de' MMMM 'de' yyyy", {
                locale: ptBR,
              })}{" "}
              • {tipo}
            </p>
              ...
              {status === "PUBLICADA" ? (
                <>
                  ...
                  <Button
                    ...
                    disabled={isPublishing || isBloqueada}
                  >
                    <Globe className="mr-2 h-4 w-4" />{" "}
                    {isPublishing ? "Processando..." : "Retornar p/ Rascunho"}
                  </Button>
                </>
              ) : (
                <>
                  ...
                  >
                    ...
                    {isGeneratingPdf ? "Gerando..." : "Pré-visualizar PDF"}
                  </Button>
                  <Button
                    variant="default"
      ...
      {isBloqueada && !isEditing && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-md p-4">
          <p className="text-yellow-800 text-sm font-medium">
            Esta escala já passou da data e não pode mais ser editada ou ter seu
            status alterado.
          </p>
        </div>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Data: {format(new Date(dataEscala), "dd/MM/yyyy")} | Tipo: {tipo} |
            Status: {status}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isEditing ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="dataEscala"
                  type="date"
                  value={format(new Date(dataEscala), "yyyy-MM-dd")}
                  onChange={(e) => setDataEscala(new Date(e.target.value))}
                />
              </div>
              <div className="space-y-2">
                ...
                <Select
                  value={tipo}
                  onValueChange={(v: TipoEscala) => setTipo(v)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div>
                ...
                <p className="font-semibold text-sm">{elaboradoPor}</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <div className="space-y-6">
          ...
          <Card key={secao.id}>
              ...
              {isEditing ? (
                <div className="space-y-2">
                  ...
                  <Input
                    value={secao.nome}
                    onChange={(e) =>
                      handleSecaoNameChange(secaoIndex, e.target.value)
                    }
            ...
            <CardContent className="space-y-4">
              ...
              {secao.itens.map((item, itemIndex) => {
                const funcoesFiltradas = funcoesProp.filter(
                  (f) =>
                    f.categoria === secao.categoriaEsperada ||
                    f.id === funcaoOutroId ||
                    !secao.categoriaEsperada,
                );
                ...
                return (
                  <div
                    key={item.id}
                    className="grid grid-cols-12 gap-x-4 gap-y-3 border p-4 rounded-md relative"
                  >
                    {isEditing ? (
                      <>
                        <Button
                          ...
                          onClick={() =>
                            handleRemoveItem(secaoIndex, itemIndex)
                          }
                        >
                        ...
                        {secao.isSecaoPalestrante ? (
                          <>
                            ...
                            <div className="col-span-12 md:col-span-4 space-y-2">
                              <Label>Auxiliar (Aluno)</Label>
                              <UserCombobox
                                users={alunos}
                                value={item.auxiliarUserId}
                                onChange={(userId) =>
                                  handleItemChange(
                                    secaoIndex,
                                    itemIndex,
                          ...
                          <>
                            <div
                              className={`col-span-12 ${isOutroSelecionado ? "md:col-span-2" : "md:col-span-3"} space-y-2`}
                            >
                              <Label>Função</Label>
                              <Select
                                value={item.funcaoId}
                            ...
                            <div className="col-span-12 md:col-span-3 space-y-2">
                              <Label>Usuário</Label>
                              <UserCombobox
                                users={secao.isSecaoAdmin ? admins : alunos}
                                value={item.userId}
                                onChange={(userId) =>
                                  handleItemChange(
                                    secaoIndex,
                        ...
                        {secao.isSecaoPalestrante ? (
                          <>
                            ...
                            <div className="col-span-6 md:col-span-3">
                              ...
                              <p className="font-medium text-sm">
                                {getAlunoName(item.userId, false)}
                              </p>
                            </div>
                            <div className="col-span-6 md:col-span-3">
                              ...
                              <p className="font-medium text-sm">
                                {item.auxiliarUserId
                                  ? getAlunoName(item.auxiliarUserId, false)
                                  : "N/A"}
                              </p>
                            </div>
                          </>
                          ...
                          <>
                            ...
                            <div className="col-span-6 md:col-span-5">
                              <Label>Usuário</Label>
                              <p className="font-medium text-sm">
                                {getAlunoName(item.userId, secao.isSecaoAdmin)}
                              </p>
                            </div>
                          </>
                        )}
              ...
              {isEditing && secao.permiteMultiplosItens && (
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => handleAddItem(secaoIndex)}
                  className="w-full border-dashed"
                >
                  <PlusCircle className="mr-2 h-4 w-4" /> Adicionar em{" "}
                  {secao.nome}
                </Button>
              )}
            </CardContent>
        ...
        {secoes.length > 0 && (
          <>
            <Card>
              <CardHeader>
                <CardTitle>Fardamento</CardTitle>
              </CardHeader>
              <CardContent>
                {isEditing ? (
                  <Textarea
                    value={fardamento}
                    onChange={(e) => setFardamento(e.target.value)}
                    rows={4}
                  />
                ) : (
