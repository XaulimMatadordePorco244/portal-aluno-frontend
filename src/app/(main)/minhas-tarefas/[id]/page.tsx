...
export default function ParecerPage({ params }: { params: { id: string } }) {
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/etapas/${params.id}/submeter-parecer`, {
                method: 'PUT',
    ...
    return (
        <div className="container mx-auto py-10 max-w-4xl">
             <div className="mb-4">
                <Link href="/minhas-tarefas" className="flex items-center text-sm text-muted-foreground hover:text-foreground">
            ...
            <Card className="mb-6">
                <CardHeader>
                    ...
                    <CardDescription>Visualizando o pedido original para emissão do parecer.</CardDescription>
                </CardHeader>
                <CardContent>
                    <p><span className="font-semibold">Assunto:</span> Assunto do processo virá aqui...</p>
                    <p className="mt-4 font-semibold">Conteúdo Original:</p>
                    <p className="whitespace-pre-wrap text-muted-foreground border p-2 rounded-md">Conteúdo do processo virá aqui...</p>
                </CardContent>
            </Card>
