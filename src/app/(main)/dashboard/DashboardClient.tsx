...
const RankingListItem = ({ rank, nome, numero, cargo, isCurrentUser }: { rank: number; nome: React.ReactNode; numero: string | null; cargo: string | null; isCurrentUser?: boolean }) => ( <div className={`border-b last:border-b-0 p-3 ${isCurrentUser ? 'bg-primary/10' : 'hover:bg-accent'}`}><div className="flex items-center gap-3"><span className={`font-bold text-lg w-8 text-center ${isCurrentUser ? 'text-primary' : 'text-muted-foreground'}`}>{rank}º</span><div><p className={`font-semibold ${isCurrentUser ? 'text-primary' : 'text-foreground'}`}>{nome}</p><p className="text-xs text-muted-foreground">{cargo} - Nº {numero || 'N/A'}</p></div></div></div> );
